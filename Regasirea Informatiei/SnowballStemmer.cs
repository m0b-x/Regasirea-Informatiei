...
public class SnowballStemmer
{
    ...
    private readonly string[] _exceptionsPart2 =
    {
        "inning", "outing", "canning", "herring", "earring",
        "proceed", "exceed", "succeed"
    };

    private readonly string[] _exceptionsRegion1 =
    {
        "gener", "arsen", "commun"
    };
    ...
    public string Stem(string word)
    {
        ...
        string excpt;
        if (_exceptions.TryGetValue(word, out excpt)) return excpt;
        ...
        if (_exceptionsPart2.Contains(word)) return word;
    ...
    {
        if (oldSuffix != null) word = word.Substring(0, word.Length - oldSuffix.Length);

        if (newSuffix != null) word += newSuffix;
    ...
    /// <summary>
    ///     The English stemmer treats apostrophe as a letter, removing it from the beginning of a word, where it might have
    ///     stood for an opening quote, from the end of the word, where it might have stood for a closing quote, or been an
    ///     apostrophe following s.
    /// </summary>
    ...
    {
        // Exceptional forms
        foreach (var except in _exceptionsRegion1.Where(word.StartsWith)) return except.Length;
    ...
    private int GetRegion(string word, int begin)
    {
        ...
        for (var i = begin; i < word.Length; i++)
        {
            ...
            if (foundVowel && IsConsonant(word[i])) return i + 1;
        }
    ...
    public bool EndsInShortSyllable(string word)
    {
        ...
        if (word.Length == 2) return IsVowel(word[0]) && IsConsonant(word[1]);
    ...
    public string Step1ARemoveOtherSPluralSuffixes(string word)
    {
        ...
        if (word.EndsWith("ied") || word.EndsWith("ies"))
        {
            ...
            if (word.Length > 4) return restOfWord + "i";
    ...
    public string Step1BRemoveLySuffixes(string word, int r1)
    {
        ...
        {
            if (SuffixInR1(word, r1, suffix)) return ReplaceSuffix(word, suffix, "ee");
        ...
        foreach (var suffix in new[] {"ed", "edly", "ing", "ingly"}.Where(word.EndsWith))
        {
            ...
            {
                if (new[] {"at", "bl", "iz"}.Any(trunc.EndsWith)) return trunc + "e";

                if (Doubles.Any(trunc.EndsWith)) return trunc.Substring(0, trunc.Length - 1);

                if (IsShortWord(trunc)) return trunc + "e";
    ...
    public string Step4RemoveSomeSuffixesInR2(string word, int r2)
    {
        foreach (var suffix in new[]
            ...
            if (word.EndsWith(suffix))
            {
                if (SuffixInR2(word, r2, suffix)) return ReplaceSuffix(word, suffix);
